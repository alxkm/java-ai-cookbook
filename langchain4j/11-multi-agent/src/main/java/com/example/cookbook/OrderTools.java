...
public class OrderTools {
    ...
    @Tool("Look up the delivery status of an order by its id, for example A-1001.")
    String orderStatus(@P("Order id in the form A-nnnn") String orderId) {
        return STATUS.getOrDefault(orderId, "unknown order");
    }

    @Tool("Return today's date. Use it whenever the user says today, tomorrow or yesterday.")
    String today() {
        return LocalDate.now().toString();
    }

    @Tool("Estimate the delivery date for an order.")
    String estimatedDelivery(@P("Order id in the form A-nnnn") String orderId) {
